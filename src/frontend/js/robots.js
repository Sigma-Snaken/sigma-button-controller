...
function renderSkeleton() {
    container.innerHTML = `<div class="card"><div class="card-header"><h2>已註冊機器人</h2><button class="btn btn-primary" id="add-robot">+ 新增機器人</button></div><div class="table-wrap"><table><thead><tr><th>名稱</th><th>IP</th><th>序號</th><th>狀態</th><th>電量</th><th>操作</th></tr></thead><tbody id="robots-body"><tr><td colspan="6" style="text-align:center;color:var(--text-muted)">載入中...</td></tr></tbody></table></div></div>`;
...
function fillRobots(robots) {
    ...
    tbody.innerHTML = robots.map(r => `<tr><td>${r.name}</td><td style="color:var(--text-muted)">${r.ip}</td><td style="color:var(--text-muted);font-size:0.78rem">${r.serial||'—'}</td><td><span class="status-dot ${r.online?'status-online':'status-offline'}"></span>${r.online?'在線':'離線'}</td><td>${fmtBattery(r.battery)}</td><td><button class="btn btn-sm btn-primary edit-robot" data-id="${r.id}" data-name="${r.name}" data-ip="${r.ip}">編輯</button> <button class="btn btn-sm btn-danger del-robot" data-id="${r.id}">刪除</button></td></tr>`).join('') || '<tr><td colspan="6" style="text-align:center;color:var(--text-muted)">尚無機器人</td></tr>';
    bindActions();
}
