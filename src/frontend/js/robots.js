...
async function renderRobots() {
    ...
    container.innerHTML = `<div class="card"><div class="card-header"><h2>已註冊機器人</h2><button class="btn btn-primary" id="add-robot">+ 新增機器人</button></div><table><thead><tr><th>名稱</th><th>IP</th><th>狀態</th><th>電量</th><th>操作</th></tr></thead><tbody>${robots.map(r => `<tr><td>${r.name}</td><td style="color:var(--text-muted)">${r.ip}</td><td><span class="status-dot ${r.online?'status-online':'status-offline'}"></span>${r.online?'在線':'離線'}</td><td>${r.battery!=null?r.battery+'%':'—'}</td><td><button class="btn btn-sm btn-primary edit-robot" data-id="${r.id}" data-name="${r.name}" data-ip="${r.ip}">編輯</button> <button class="btn btn-sm btn-danger del-robot" data-id="${r.id}">刪除</button></td></tr>`).join('')}${robots.length===0?'<tr><td colspan="5" style="text-align:center;color:var(--text-muted)">尚無機器人</td></tr>':''}</tbody></table></div>`;
    container.querySelector('#add-robot').onclick = () => {
        showModal('新增機器人', '<div class="form-group"><label>ID</label><input id="r-id" placeholder="kachaka-1"></div><div class="form-group"><label>名稱</label><input id="r-name" placeholder="大廳機器人"></div><div class="form-group"><label>IP</label><input id="r-ip" placeholder="192.168.50.101"></div>', async () => {
            try { await api.createRobot({id:document.getElementById('r-id').value,name:document.getElementById('r-name').value,ip:document.getElementById('r-ip').value}); showToast('機器人已新增'); await renderRobots(); } catch(e) { showToast(e.message,'error'); }
        });
    };
