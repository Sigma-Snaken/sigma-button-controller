...
async function renderButtons() {
    ...
    container.innerHTML = `<div class="card"><div class="card-header"><h2>已配對按鈕</h2><button class="btn btn-success" id="start-pair">開始配對</button></div><table><thead><tr><th>名稱</th><th>IEEE</th><th>電量</th><th>最後回報</th><th>操作</th></tr></thead><tbody>${buttons.map(b => `<tr><td>${b.name||b.ieee_addr}</td><td style="color:var(--text-muted);font-size:0.78rem">${b.ieee_addr}</td><td>${b.battery!=null?b.battery+'%':'—'}</td><td style="color:var(--text-muted);font-size:0.78rem">${b.last_seen||'—'}</td><td><button class="btn btn-sm btn-primary rename-btn" data-id="${b.id}" data-name="${b.name||''}">重命名</button> <button class="btn btn-sm btn-danger del-btn" data-id="${b.id}">移除</button></td></tr>`).join('')}${buttons.length===0?'<tr><td colspan="5" style="text-align:center;color:var(--text-muted)">尚無配對按鈕</td></tr>':''}</tbody></table></div><div class="pair-zone hidden" id="pair-zone"><h4 style="color:var(--success)">配對模式啟動中...</h4><p style="font-size:0.82rem;color:var(--text-muted);margin-top:0.5rem">請長按 SNZB-01 按鈕 5 秒直到 LED 閃爍</p><p style="margin-top:0.5rem;font-size:0.82rem;color:var(--success)" id="pair-countdown"></p><button class="btn btn-danger" style="margin-top:0.75rem" id="stop-pair">停止配對</button></div>`;
