...
2. 開啟瀏覽器，前往 **`http://sigma.local:8500`**
3. 切換到「**WiFi 設定**」分頁
...
3. 輸入密碼，點擊連線
4. 連線成功後，AP 模式會自動關閉
5. Pi 取得新 IP 後，可透過 `http://sigma.local:8500` 或新 IP 位址存取
...
- **mDNS**：`http://sigma.local:8500`（推薦，IP 變動也不受影響）
- **IP 位址**：例如 `http://192.168.50.6:8500`（可在 WiFi 狀態中查看）
...
| 元件 | 說明 | Port |
|------|------|------|
| Sigma App | 主應用程式（FastAPI） | 8500 |
| Mosquitto | MQTT 訊息代理 | 1883 |
| Zigbee2MQTT | Zigbee 橋接器 | 8501 |
...
2. 嘗試 `http://sigma.local:8500`（mDNS）
...
2. 確認 Zigbee2MQTT 是否正常運行：`http://sigma.local:8501`
...
>
> **Online 路線重啟特例**：Pi 重啟時若有 Online 路線正在執行，會被標記為「**已中斷 (interrupted)**」終態（不可續跑、不會自動恢復）。機器人可能停在某一站、身上還帶著貨架 — 需現場確認並手動處理（推回原位或執行 `reset_shelf`）。
>
> **Offline 路線重啟特例**：Pi 重啟時若有處於「離線執行中」狀態的路線，會由 `_clean_stale_offline_running` 自動標記為「已完成」（因為無法重新接管 SSH 連線中的腳本）；但機器人 Playground 容器內的腳本不受 Pi 重啟影響，會把整段路線跑完 + 把貨架歸還 + 回到充電座，並嘗試 10 分鐘內回報結果。
